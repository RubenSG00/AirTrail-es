<script lang="ts">
  ...
  let {
    value = $bindable(null),
    placeholder = 'Buscar aeropuerto por nombre o código',
    disabled = false,
    compact = false,
    onchange,
    onCreateNew,
      ...
      >
            ...
            <div class="flex flex-row gap-2 justify-between">
              <div class="flex flex-col overflow-hidden">
                <span class={cn('truncate', { 'text-sm font-medium': compact })}
                  >{toTitleCase(airport.name)}</span
                >
                <p class={cn('text-sm', { 'text-xs': compact })}>
                  {#if airport.iata}
              ...
              <div
                class={cn('shrink-0', { 'w-12': !compact, 'w-10': compact })}
              >
                <img
                  src="https://flagcdn.com/{airport.country.toLowerCase()}.svg"
                  alt={airport.country}
                  class={cn('aspect-video h-full', { 'rounded-sm': compact })}
                />
              </div>
            </div>
          </li>
        {:else}
          {#if loading || !$inputValue}
            ...
            >
              {#if loading}
                {compact ? 'Buscando...' : 'Buscando aeropuertos...'}
              {:else}
          ...
          {:else}
            <li
              class="relative cursor-default scroll-my-2 rounded-md p-2 bg-popover dark:bg-dark-1 border text-sm text-muted-foreground"
            >
              No se encontraron {compact ? 'resultados' : 'aeropuertos'}
            </li>
          {/if}
        {/each}
      </div>
