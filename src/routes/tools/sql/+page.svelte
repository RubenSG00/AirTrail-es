...
<div class="container h-full flex flex-col items-center justify-center gap-2">
  <h1 class="text-2xl font-medium">Consola SQL</h1>
  <Textarea bind:value={query} placeholder="Tu consulta SQL" />
      ...
      <div class="max-h-[80dvh] overflow-y-auto w-full rounded-md border">
        <Table.Root>
          ...
          <Table.Body>
            ...
            {:else}
              <Table.Row>
                <Table.Cell colspan={result.rows.length}>Sin datos</Table.Cell>
              </Table.Row>
            {/each}
          </Table.Body>
        </Table.Root>
